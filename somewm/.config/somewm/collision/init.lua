...
local util         = require( "gears.table"     )
...
local function start_loop(is_swap,is_max)
  capi.keygrabber.run(function(mod, key, event)
    ...
    elseif key == "Control_L" or key == "Control_R" then
      is_max = event == "press"
      return #mod > 0 and util.hasitem(mod,"Mod4") or exit_loop()
...
local function new(k)
  ...
  glib.idle_add(glib.PRIORITY_DEFAULT_IDLE, function()
    ...
    capi.root._keys(util.join(capi.root._keys(),unpack(aw)))
  end)
