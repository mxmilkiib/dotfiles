...
# IRC-style buffer: push lines into history without executing, recall later
# usage: type command, press alt-down to save to history, continue typing
fake-accept-line() {
	# if the Zle buffer is not empty
	if [[ -n "$BUFFER" ]]; then
		# save current buffer to history without executing
		print -S "$BUFFER"
	fi
	return 0
}
...
# wrapper for down-line that saves buffer when at end of history
down-or-fake-accept() {
	# if at the last history line and no newline in right buffer
	if (( HISTNO == HISTCMD )) && [[ "$RBUFFER" != *$'\n'* ]]; then
		zle fake-accept-line
	fi
	zle down-line-or-history "$@"
}
zle -N down-or-fake-accept

# bind to alt-down (history-substring-search uses regular down arrow)
bindkey '^[[1;3B' down-or-fake-accept  # alt-down: save current line to history

# debug helper: show raw key codes to diagnose binding issues
# usage: type 'showkey', press the key combo, then ctrl-c
alias showkey='cat -v'
...
# print underlined and italicized text, with normal foreground, and blue background
...
# quick shortcut for tmux (see function below)
# alias t='tmux'

# check if tmux server exists, attach if yes, create if no
function t(){
...
# make xargs expand zsh aliases
alias xargs='xargs '

# make sudo expand zsh aliases
alias sudo='sudo '

# change to root with user's environment
...
# info supersedes man feature-wise, but requires learning new hotkeys
# alias man='info'

# display basic machine details
alias wtf='hostname && cat /etc/*-release && whoami && pwd'

# display list of handy system info commands
...
# graphical tree of subdirectories
#alias 'lt=tree -d'
...
# update last working directory when directory changes
...
# automatically jump to last working directory on first load
# if [[ -z "$ZSH_LAST_WORKING_DIRECTORY" ]]; then
	# lwd 2>/dev/null && ZSH_LAST_WORKING_DIRECTORY=1 || true
# fi


# browse path web browser style (alternative implementation)
# up() { pushd .. > /dev/null; }
# down() { popd > /dev/null; }

up() { cd $(eval printf '../'%.0s {1..$1}) && pwd; }
# use "cd -[N]" or "cd -<TAB>" to get back

# change into directory and long list files
cl() { cd "$1" && ll . ; }
...
bak() { cp "$1" "$1".bak; }
# backup file and remove original (move to .bak)
bakd() { mv "$1" "$1".bak; }
...
# make parent directories if they don't exist (mkdir -p behavior)
alias mkdir='mkdir -p'
...
# make a backup of a file with .bak extension
mkbk() {
  cp "$1"{,.bak}
}
...
spaces2underscores() {
  ...
  # couldn't make it work with the quotes
...
# display mounts with column output
alias mounts='mount | column -t'

# human-readable df by default
alias 'df=df -h'

# dfc sorted by type, showing used size, type, wide filename, wide bar, sum usage for ext/fat/nfs/btrfs/tmpfs
alias dfcc="dfc -q type -dTws -t ext,vfat,nfs,btrfs,tmpfs"

# cdu with interactive, depth, human-readable flags
alias du2='cdu -idh'

# list directory items by size
alias 'dus=du -ms * | sort -n'

# ncdu with dark color scheme and quiet mode
alias 'ncdu=ncdu --color dark -q'

# check disk usage in ncdu for arch root, excluding common non-data directories
...
# check logical disk usage in btdu with expert mode
...
# mtr with enhanced display options (ping and traceroute combo)
alias mtr="mtr -b -z -o LSDRNABWVGJMXI"

# quick local HTTP server
alias serverphp='php -S localhost:8000'

# list open TCP ports and their processes
...
# ytcracker nerdcore reference
alias whodis='whois'

# get shit done - temporarily redirect distracting sites to localhost
alias gsd='sudo /home/milk/scripts/gsd.sh/gsd.sh'

# continuously ping SSH port to check connectivity
sshping() {
   while sleep 0.5; do
...
# install specific package with editmenu
...
# download PKGBUILD
alias pG='yay -G'
# make package, get dependencies and install
alias mP='makepkg -si'
# update .SRCINFO
alias mS='makepkg --printsrcinfo > .SRCINFO'
# update .SRCINFO, git add, commit and push
...
# required pkg: vimpager. use vim and plugins to view a file
alias vp=vimpager
...
# easy git commit with message from arguments
function gc(){
	git commit -m "$*";
}
...
# color diff (commented out, colordiff alternative)
# diff () {
...
# alsamixer with both input and output
alias alsamixer='alsamixer -V=all'

alias ncm='ncmpcpp'

# refresh pulseaudio devices by reloading udev module
...
# shortcut function that simplifies usage of xclip
...
# aliases and functions leveraging the cb() function
...
# URL-encode string and print it
function url-encode; {
				setopt extendedglob
				echo "${${(j: :)@}//(#b)(?)/%$[[##16]##${match[1]}]}"
}
...
# switch to left-handed mouse and back
...
# reload compton compositor
alias comptonR='sudo killall compton; compton -b'
alias picomR='sudo killall picom; picom -b'

# xev showing only keyboard events
alias xevk='xev -event keyboard'

### misc utilities

# wrapper for READNULLCMD in environment.zsh
function less_rfx() {
	less -RFX "$@"
}

# add together multiple 24h time ranges
...
### -g global aliases (work at end of line)
...
# grep with ripgrep, case-insensitive, line numbers
alias -g G='| rg -in'
# for piping
alias -g GP='| rg -in --color=never'
# run app in background without stdout/stderr
alias -g S='&> /dev/null &'
# fzf
alias -g F='`fzf`'
# vim
