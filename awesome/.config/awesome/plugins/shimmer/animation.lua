...
-- progression mode cycling order
local progression_modes_list = {"uniform", "wave", "drift", "scatter", "chaos", "random"}
local current_progression_index = 2  -- start with wave

-- preset cycling state
local preset_list = {"gold_contrast", "bright_gold", "warm_amber", "warm_light", "honey_glow", "candy", "candy_pastel", "amber_pulse", "cloud", "copper", "electric_buzz", "sepia_soft", "deep_gold", "plasma_drift", "aurora_scatter", "digital_chaos", "debug", "static_amber", "border_sync", "off"}
...
local function makeColorGradient(frequency1, frequency2, frequency3, phase1, phase2, phase3, center, width, length)
    ...
    for i = 0, length - 1 do
        ...
        palette[i + 1] = string.format("#%02x%02x%02x", r, g, b)  -- 1-based indexing
    end
    return palette
end
...
local function makeGoldShinePalette(length, opts)
    ...
    local val_min = opts.value_min or 0.68      -- raise floor to avoid too-dark dips
    local val_max = opts.value_max or 0.96      -- cap highlights to avoid near-white
    ...
    for i = 1, length do
        ...
        local sat = sat_base - sat_var * math.sin(2 * math.pi * t * sat_freq)  -- tiny sat wobble
...
local shimmer_config = {
    ...
    -- warm amber gradient - readable, restrained range with amber tinge
    warm_amber = {
        speed = 0.35,
        per_character_default = false,
        gradient = {
    ...
    -- rapid electric buzz - high-energy, attention-grabbing
    ...
    -- copper shine - warmer, reddish gold family
    copper = {
        speed = 0.40,
        per_character_default = false,
        use_gold_shine = true,
    ...
    -- sepia soft - low-key sepia gradient
    sepia_soft = {
        speed = 0.32,
        per_character_default = false,
        gradient = {
    ...
    -- golden hour warm light - gentle, readable with golden hour tones
    -- produces soft golden-yellow shimmer reminiscent of sunset light
    warm_light = {
        speed = 0.30,
        per_character_default = false,
        gradient = {
    ...
    for i, char in ipairs(chars) do
        if char:match("%s") then
            ...
            if mode == per_character_modes.wave then
                -- wave effect: each character progressively offset
                letter_phase = letter_phase + (i * 1.2)  -- reduced for slower animation
            elseif mode == per_character_modes.random then
                ...
                letter_phase = letter_phase + (math.random() * 15.0)  -- reduced range for slower animation
            ...
            elseif mode == per_character_modes.chaos then
                -- high randomness, granular-like sampling
                math.randomseed(text_seed + i)
                letter_phase = letter_phase + (math.random() * 5.0)
            end
...
-- core color calculation with reverse cycling for seamless loops
function M.get_color(mode_config, text_length, phase_offset)
    ...
    local effective_speed = config.speed * global_speed_multiplier * 0.05
    local base_time = shimmer_step * effective_speed * animation_direction
    local length_factor = (text_length or 1) * 0.1
    local phase_factor = (phase_offset or 0) * 2.0
    local animation_phase = base_time + length_factor + phase_factor
    
    -- ping-pong animation: forward then reverse to avoid jumps
    -- ensure we have enough colors for the animation cycle
    if #palette < 2 then
        return palette[1] or base_gold
    end
    
    local cycle_length = #palette * 2 - 2  -- account for reverse path
    local raw_index = math.floor(math.abs(animation_phase) * 20) % cycle_length
    local palette_index
...
function M.apply_to_widget(widget, text, status_symbols, options)
    ...
    if use_per_character then
        ...
        -- when per-character is off, ignore phase_offset to keep all widgets synchronized
        local shimmer_color = M.get_color(nil, #display_text, 0)
